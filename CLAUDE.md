...
2. **AI processing** (`app/services/ai_engine.py`, fully async via `AsyncOpenAI`/`httpx.AsyncClient`) — `process_post()` runs `classify_post()`, `summarise_post()`, `extract_events()` (three DeepSeek calls) and `generate_embedding()` (Ollama BGE-M3) concurrently via `asyncio.gather(..., return_exceptions=True)`. Uses `deepseek-chat` via the OpenAI SDK (`base_url="https://api.deepseek.com"`). `<thought>` blocks are stripped by regex before parsing. Each of the four calls fails independently — one DeepSeek/Ollama hiccup no longer loses the whole post: a failed category falls back to `"Прочее"`, a failed summary falls back to truncated raw text (so the post still has a non-null `summary` and appears in the digest), failed events fall back to `[]`, and a failed embedding falls back to a zero-vector. The post is always stored with whatever succeeded, instead of being silently dropped.
