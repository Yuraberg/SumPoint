...
from app.utils.text import split_for_telegram
...
async def send_digest_for_user(
    ...
    for i, part in enumerate(parts, start=1):
        body = f"{part}\n\n_Часть {i}/{total}_" if total > 1 else part
        try:
            await bot.send_message(chat_id=user_id, text=body, parse_mode="Markdown")
            ...
            logger.warning(
                "Digest part %d/%d Markdown rejected for user %s (%s); resending as plain text",
                i, total, user_id, e,
            )
            try:
